...
@section('title', 'Detail Rincian Biaya Perjalanan Dinas')
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    ...
    <div class="flex justify-between items-center p-6 border-b border-gray-200">
        ...
        <div class="flex gap-2">
            <a href="{{ route('rincian.edit', $rincian->id) }}" 
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-gray-800 font-medium">{{ $rincian->nomor }}</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-2">Tanggal</label>
                <p class="text-gray-800 font-medium">{{ \Carbon\Carbon::parse($rincian->tanggal)->format('d F Y') }}</p>
            </div>
            <div class="md:col-span-2 bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-gray-800 font-medium">{{ $rincian->tujuan }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full border border-gray-300 rounded-lg">
                    ...
                    <tbody>
                        @php $subtotalHarian = 0; @endphp
                        @foreach($rincian->pegawai as $index => $pegawai)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-2 text-sm text-gray-500 border-b">{{ $index + 1 }}</td>
                            <td class="px-4 py-2 text-sm text-gray-800 border-b">{{ $pegawai['nama'] }}</td>
                            <td class="px-4 py-2 text-sm text-gray-600 border-b">{{ $pegawai['nip'] ?? '-' }}</td>
                            <td class="px-4 py-2 text-sm text-right text-gray-600 border-b">Rp {{ number_format($pegawai['nominal']) }}</td>
                            <td class="px-4 py-2 text-sm text-right text-gray-600 border-b">{{ $pegawai['hari'] }} hari</td>
                            <td class="px-4 py-2 text-sm text-right font-semibold text-gray-800 border-b">
                                Rp {{ number_format($pegawai['nominal'] * $pegawai['hari']) }}
                                @php $subtotalHarian += $pegawai['nominal'] * $pegawai['hari']; @endphp
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                    <tfoot class="bg-gray-100">
                        <tr>
                            ...
                            <td class="px-4 py-2 text-right font-bold text-blue-600">Rp {{ number_format($subtotalHarian) }}</td>
                        </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p class="text-gray-800 font-medium">{{ $rincian->bendahara['nama'] ?? '-' }}</p>
                <p class="text-sm text-gray-500 mt-1">NIP: {{ $rincian->bendahara['nip'] ?? '-' }}</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                <label class="block text-xs font-semibold text-gray-500 uppercase mb-2">Kepala Dinas</label>
                <p class="text-gray-800 font-medium">{{ $rincian->kepala_dinas['nama'] ?? 'BUDI ANDRIAN SUTANTO, S. Sos., M.M' }}</p>
                <p class="text-sm text-gray-500 mt-1">NIP: {{ $rincian->kepala_dinas['nip'] ?? '19760218 200701 1 006' }}</p>
            </div>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="bg-white rounded-lg p-3 border border-gray-200">
                    ...
                    <div class="text-xl font-bold text-blue-600">{{ count($rincian->pegawai) }}</div>
                    <div class="text-xs text-gray-400">orang</div>
                </div>
                <div class="bg-white rounded-lg p-3 border border-gray-200">
                    <div class="text-xs text-gray-500">Total Uang Harian</div>
                    <div class="text-xl font-bold text-green-600">Rp {{ number_format($subtotalHarian) }}</div>
                    <div class="text-xs text-gray-400">(pegawai × nominal × hari)</div>
                </div>
                <div class="bg-white rounded-lg p-3 border border-gray-200">
                    <div class="text-xs text-gray-500">Uang Transport</div>
                    <div class="text-xl font-bold text-orange-600">Rp {{ number_format($rincian->transport) }}</div>
            ...
            <div class="bg-blue-50 rounded-lg p-3 border border-blue-200">
                <div class="flex justify-between items-center">
                    ...
                    <div class="text-2xl font-bold text-blue-800">Rp {{ number_format($rincian->total) }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
            ...
            <p class="text-gray-800 italic">{{ $rincian->terbilang }}</p>
        </div>
    </div>
