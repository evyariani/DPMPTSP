...
<div class="mb-6">
    <div class="flex justify-between items-center">
        <div>
            <h2 class="text-lg font-semibold text-gray-700">Surat Perintah Tugas</h2>
            <p class="text-gray-500">Kelola data Surat Perintah Tugas</p>
        </div>
        <a href="/spt/create" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center transition duration-200">
...
<div class="bg-white rounded-lg shadow p-4 mb-6">
    <form method="GET" action="/spt" class="flex items-center space-x-4">
        <div class="flex-1">
            <input type="text" name="search" placeholder="Cari data surat perintah tugas..." 
        ...
        @if(request()->has('search') || request()->has('level'))
            <a href="/spt" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg transition duration-200">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">sptname</th>
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <div class="flex items-center">
                            <div class="flex-shrink-0 h-10 w-10">
                                <div class="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                                    <span class="text-blue-600 font-semibold">{{ strtoupper(substr($spt->sptname, 0, 1)) }}</span>
                                </div>
                            </div>
                            <div class="ml-4">
                                <div class="text-sm font-medium text-gray-900">{{ $spt->sptname }}</div>
                                <div class="text-sm text-gray-500">ID: {{ $spt->id }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @php
                            ...
                            $color = $levelColors[$spt->level] ?? 'bg-gray-100 text-gray-800';
                        @endphp
                        <span class="px-3 py-1 rounded-full text-xs font-medium {{ $color }}">
                            {{ ucfirst(str_replace('_', ' ', $spt->level)) }}
                        </span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {{ $spt->created_at?->format('d/m/Y H:i') ?? '-' }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            <a href="/spt/{{ $spt->id }}/edit" 
                               class="text-blue-600 hover:text-blue-900 px-3 py-1 rounded hover:bg-blue-50 transition duration-150"
                               title="Edit spt">
                                <i class="fas fa-edit mr-1"></i> Edit
                            </a>
                            @if($spt->id != session('spt')['id'])
                            <form action="/spt/{{ $spt->id }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                                        onclick="return confirm('Apakah Anda yakin ingin menghapus spt {{ $spt->sptname }}?')"
                ...
                <tr>
                    <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                        <div class="flex flex-col items-center justify-center">
                            <i class="fas fa-spts text-gray-300 text-4xl mb-3"></i>
                            <p class="text-lg">Tidak ada data spt</p>
                            <p class="text-sm mt-1">Mulai dengan menambahkan spt baru</p>
                            <a href="/spt/create" class="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg inline-flex items-center">
