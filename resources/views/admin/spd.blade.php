        ...
        <div class="flex flex-col md:flex-row md:items-center space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @if(request()->hasAny(['search', 'bulan', 'tahun']))
                    <a href="{{ route('spd.index') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-800 px-4 py-2 rounded-lg transition duration-200 flex items-center">
        ...
        <div class="mt-4 pt-3 border-t border-gray-200">
            <div class="flex items-center flex-wrap gap-2">
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    ...
                    @endphp
                    {{ $bulanNames[request('bulan')-1] ?? request('bulan') }}
                    <a href="{{ request()->fullUrlWithQuery(['bulan' => null]) }}" class="ml-2 text-green-600 hover:text-green-800">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
                @endif
                ...
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                    <i class="fas fa-calendar-year mr-1"></i> {{ request('tahun') }}
                    <a href="{{ request()->fullUrlWithQuery(['tahun' => null]) }}" class="ml-2 text-purple-600 hover:text-purple-800">
                        <i class="fas fa-times"></i>
                    </a>
                </span>
                @endif
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-6 py-4">
                        <span class="spd-badge"><i class="fas fa-file-alt mr-1 text-xs"></i> {{ Str::limit($spd->nomor_surat, 25) }}</span>
                        @if(strlen($spd->nomor_surat) > 25)
                            <button onclick="showFullText(this, '{{ addslashes($spd->nomor_surat) }}', 'Nomor Surat')" class="text-blue-500 text-xs mt-1 block hover:underline">Lihat selengkapnya</button>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4">
                        @if($spd->penggunaAnggaran)
                            <div class="flex items-center gap-2">
                                ...
                                <div class="min-w-0">
                                    <div class="text-sm font-medium text-gray-900 tooltip" title="{{ $spd->penggunaAnggaran->nama }}">{{ Str::limit($spd->penggunaAnggaran->nama, 20) }}</div>
                                    <div class="text-xs text-gray-500 tooltip" title="{{ $spd->penggunaAnggaran->jabatan ?? '-' }}">{{ Str::limit($spd->penggunaAnggaran->jabatan ?? '-', 20) }}</div>
                                </div>
                            </div>
                        @else
                        ...
                        @if(count($pelaksanaList) > 0)
                            <div class="flex items-center gap-1">
                                <div class="flex -space-x-2">
                                    @foreach(array_slice($pelaksanaList, 0, 3) as $pel)
                                        <div class="w-7 h-7 rounded-full bg-indigo-100 border-2 border-white flex items-center justify-center text-xs font-medium text-indigo-600 tooltip" 
                                             title="{{ $pel['nama'] ?? '' }}{{ isset($pel['nip']) && $pel['nip'] ? ' - ' . $pel['nip'] : '' }}">
                                            {{ strtoupper(substr($pel['nama'] ?? '?', 0, 1)) }}
                                        </div>
                                    @endforeach
                                </div>
                                <span class="text-xs text-gray-500 ml-1">{{ count($pelaksanaList) }} orang</span>
                                @if(count($pelaksanaList) > 3)
                                    <button type="button" onclick="showPelaksanaDetail({{ json_encode($pelaksanaList) }})" class="text-blue-500 text-xs hover:underline">detail</button>
                                @endif
                            </div>
                        @else
                            <span class="text-gray-400 text-sm">-</span>
                    ...
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-700 line-clamp-2 tooltip" title="{{ $spd->maksud_perjadin }}">
                            {{ Str::limit($spd->maksud_perjadin, 50) }}
                        </div>
                        @if(strlen($spd->maksud_perjadin) > 50)
                            <button type="button" onclick="showFullText(this, '{{ addslashes($spd->maksud_perjadin) }}', 'Maksud Perjalanan Dinas')" class="text-blue-500 text-xs mt-1 hover:underline">Lihat selengkapnya</button>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($spd->tanggal_berangkat && $spd->tanggal_kembali)
                            <div class="text-sm text-gray-900">{{ \Carbon\Carbon::parse($spd->tanggal_berangkat)->format('d/m/Y') }} → {{ \Carbon\Carbon::parse($spd->tanggal_kembali)->format('d/m/Y') }}</div>
                            <div class="text-xs text-blue-600 mt-1">{{ $spd->lama_perjadin }} Hari</div>
                        @else
                            <span class="text-gray-400 text-sm">-</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('spd.edit', $spd->id_spd) }}" class="text-green-600 hover:text-green-900 p-1 rounded hover:bg-green-50 transition duration-150 tooltip" title="Edit SPD">
                                <i class="fas fa-edit"></i>
                                <span class="tooltip-text">Edit SPD</span>
                            </a>
                            <a href="{{ route('spd.belakang', $spd->id_spd) }}" class="text-indigo-600 hover:text-indigo-900 p-1 rounded hover:bg-indigo-50 transition duration-150 tooltip" title="Halaman Belakang">
                                <i class="fas fa-file-alt"></i>
                                <span class="tooltip-text">Halaman Belakang</span>
                            </a>
                            <a href="{{ route('spd.preview-depan', $spd->id_spd) }}" target="_blank" class="text-blue-600 hover:text-blue-900 p-1 rounded hover:bg-blue-50 transition duration-150 tooltip" title="Preview PDF Depan">
                ...
                <tr>
                    <td colspan="7" class="px-6 py-12 text-center text-gray-500">
                        <div class="flex flex-col items-center">
                            ...
                            <a href="{{ route('spt.index') }}" class="mt-3 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg inline-flex items-center transition duration-200">
...
function exportData() {
    ...
    window.location.href = "{{ route('spd.export') }}?" + params.toString();
