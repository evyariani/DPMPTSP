...
<form method="POST" action="{{ route('rincian.update', $rincian->id) }}" id="formRincian">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                    ...
                    <div class="font-medium text-gray-800 mt-1">
                        ...
                        {{ $rincian->tanggal_berangkat ? $rincian->tanggal_berangkat->format('d/m/Y') : '-' }}
                    </div>
                </div>
                <div>
                    ...
                    <div class="font-medium text-gray-800 mt-1">
                        <i class="fas fa-calendar-alt text-gray-400 mr-1"></i>
                        {{ $rincian->tanggal_kembali ? $rincian->tanggal_kembali->format('d/m/Y') : '-' }}
                    </div>
                </div>
                <div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 font-mono">{{ $peg['nip'] ?? '-' }}</td>
                        <td class="px-6 py-4 text-sm text-gray-600">{{ $peg['jabatan'] ?? '-' }}</td>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div class="p-6">
            ...
            @endphp
            @if($bendahara && ($bendahara->nama ?? $rincian->bendahara_nama))
                <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div>
                        <div class="font-semibold text-gray-800 text-lg">{{ $bendahara->nama ?? $rincian->bendahara_nama }}</div>
                        <div class="text-sm text-gray-500 mt-1">
                            <i class="fas fa-id-card mr-1"></i> NIP: {{ $bendahara->nip ?? $rincian->bendahara_nip ?? '-' }}
                        </div>
                        <div class="text-sm text-gray-500">
                            <i class="fas fa-briefcase mr-1"></i> Jabatan: {{ $bendahara->jabatan ?? $rincian->bendahara_jabatan ?? '-' }}
                        </div>
                        @if($rincian->bendahara_nama)
            ...
            @elseif($rincian->bendahara_pengeluaran_id && $rincian->bendaharaPengeluaran)
                <div class="flex items-center gap-4 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                    ...
                    <div>
                        <div class="font-semibold text-gray-800 text-lg">{{ $rincian->bendaharaPengeluaran->nama }}</div>
                        <div class="text-sm text-gray-500 mt-1">
                            <i class="fas fa-id-card mr-1"></i> NIP: {{ $rincian->bendaharaPengeluaran->nip ?? '-' }}
                        </div>
                        <div class="text-sm text-gray-500">
                            <i class="fas fa-briefcase mr-1"></i> Jabatan: {{ $rincian->bendaharaPengeluaran->jabatan ?? '-' }}
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div class="text-2xl font-bold text-blue-600 mt-1">
                        {{ count($pegawaiList) }} <span class="text-sm font-normal text-gray-500">orang</span>
                    </div>
                </div>
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    <div class="text-xs text-gray-500">
                ...
                <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                    ...
                    <div class="text-2xl font-bold text-purple-600 mt-1">
                        {{ $rincian->lama_perjadin ?? 0 }} <span class="text-sm font-normal text-gray-500">hari</span>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                    <div class="flex justify-between items-center">
                        ...
                        <div class="text-2xl font-bold text-blue-800" id="grandTotal">
                            Rp {{ number_format($rincian->total_uang_harian_keseluruhan ?? $rincian->total, 0, ',', '.') }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden mb-6">
        ...
        <div class="p-6">
            <textarea name="terbilang" 
                      ...
                      readonly>{{ $rincian->terbilang ?? '' }}</textarea>
    ...
    <div class="flex justify-end gap-3">
        <a href="{{ route('rincian.index') }}" class="px-6 py-3 bg-gray-300 hover:bg-gray-400 text-gray-800 rounded-lg transition duration-200 flex items-center">
