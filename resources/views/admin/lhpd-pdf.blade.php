...
<table class="content-table">
    @foreach($dasarList as $index => $dasarItem)
        @if($index == 0)
        <tr>
            ...
            <td class="content-col" colspan="2">{{ $dasarItem }}</td>
        </tr>
        @else
        <tr>
...
<table class="content-table">
    <tr>
        ...
        <td class="content-col" colspan="3">
            Perjalanan Dinas dilaksanakan pada tanggal <strong>{{ $tanggalBerangkatStr ?: '________' }}</strong> dengan tujuan <strong>{{ $tujuanDaerah ?: '________' }}</strong>
        </td>
    </tr>
</table>
...
<table class="content-table">
    <tr>
        ...
        <td class="titikdua-col">:</td>
        <td class="content-col" colspan="3">
            @if(count($hasilList) > 1)
                <ul class="hasil-list">
                    @foreach($hasilList as $hasilItem)
                        <li>{{ $hasilItem }}</li>
                    @endforeach
                </ul>
            @else
                <div class="hasil-text">{{ $hasilList[0] ?: '___' }}</div>
            @endif
        </td>
    </tr>
</table>
...
<div class="tanda-tangan">
    <div class="tempat-tanggal">
        {{ $tempat ?: '________' }}@if($tempat && $tanggalLaporanStr), @endif{{ $tanggalLaporanStr ?: '________' }}
    </div>
    <div>Yang Membuat Laporan,</div>
    <div style="height: 30px;"></div>

    @if($pegawaiCollection && $pegawaiCollection->count() > 0)
        @foreach($pegawaiCollection as $idx => $pegawai)
            @if($idx > 0)
                <div style="margin-top: 15px;"></div>
            @endif
            <div class="nama-ttd">{{ $pegawai->nama ?? ($pegawai['nama'] ?? '_________________________') }}</div>
            <div class="nip-ttd">
                @php
