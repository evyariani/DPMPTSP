        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $item->tahun_anggaran }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600 font-mono">{{ $item->kode_rekening }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600 max-w-xs truncate">{{ $item->sub_kegiatan }}</td>
                    <td class="px-4 py-3 text-sm text-right font-semibold text-gray-800">Rp {{ number_format($item->nominal, 0, ',', '.') }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">{{ \Carbon\Carbon::parse($item->tanggal_kwitansi)->format('d/m/Y') }}</td>
                    <td class="px-4 py-3 text-center">
                        <div class="flex items-center justify-center gap-1">
                            ...
                            <!-- Hapus -->
                            <form action="{{ route('kwitansi.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus kwitansi ini?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                ...
                <tr>
                    <td colspan="7" class="px-4 py-12 text-center text-gray-400">
                        ...
                        <a href="{{ route('kwitansi.create') }}" class="text-blue-500 text-sm mt-2 inline-block">Tambah kwitansi pertama</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
