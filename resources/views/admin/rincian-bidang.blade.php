        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    ...
                    <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $item->nomor }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">{{ \Carbon\Carbon::parse($item->tanggal)->format('d/m/Y') }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">{{ $item->tujuan }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">
                        <div class="flex -space-x-2">
                            @foreach(array_slice($item->pegawai, 0, 2) as $peg)
                                <div class="w-7 h-7 rounded-full bg-blue-100 border-2 border-white flex items-center justify-center text-xs font-medium text-blue-600" title="{{ $peg['nama'] }}">
                                    {{ substr($peg['nama'], 0, 1) }}
                                </div>
                            @endforeach
                            @if(count($item->pegawai) > 2)
                                <div class="w-7 h-7 rounded-full bg-gray-100 border-2 border-white flex items-center justify-center text-xs font-medium text-gray-600" title="{{ count($item->pegawai) - 2 }} pegawai lainnya">
                                    +{{ count($item->pegawai) - 2 }}
                                </div>
                            @endif
                        </div>
                    </td>
                    <td class="px-4 py-3 text-sm text-right text-gray-600">Rp {{ number_format($item->transport) }}</td>
                    <td class="px-4 py-3 text-sm text-gray-600">
                        @php
                            $bendahara = $item->pegawai[0] ?? null;
                        @endphp
                        @if($bendahara)
                            <div class="text-sm font-medium text-gray-800">{{ $bendahara['nama'] }}</div>
                            <div class="text-xs text-gray-500">{{ $bendahara['nip'] ?? '-' }}</div>
                        @else
                    ...
                    <td class="px-4 py-3 text-sm text-right font-semibold text-gray-800">Rp {{ number_format($item->total) }}</td>
                ...
                <tr>
                    <td colspan="10" class="px-4 py-12 text-center text-gray-400">
                        ...
                        <a href="{{ route('rincian.create') }}" class="text-blue-500 text-sm mt-2 inline-block">Tambah data pertama</a>
                    </td>
                </tr>
                @endforelse
            </tbody>
