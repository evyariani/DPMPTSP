...
@section('title', 'Edit Laporan Hasil Perjalanan Dinas (LHPD)')
...
<div class="mb-6">
    <div class="flex justify-between items-center">
        ...
        <div>
            <a href="{{ route('lhpd.index') }}" class="btn-secondary btn">
...
<!-- Form Edit LHPD -->
<form action="{{ route('lhpd.update', $lhpd->id_lhpd) }}" method="POST" enctype="multipart/form-data" id="formEditLhpd">
    @csrf
    @method('PUT')
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           value="{{ $lhpd->tanggal_berangkat ? \Carbon\Carbon::parse($lhpd->tanggal_berangkat)->format('d/m/Y') : '-' }}"
    ...
    <div class="info-card">
        ...
        <div class="p-6">
            <!-- Foto yang sudah ada -->
            @if(isset($existingFotos) && count($existingFotos) > 0)
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    Foto yang sudah ada ({{ count($existingFotos) }})
                </label>
                <div class="foto-grid" id="existing-fotos-container">
                    @foreach($existingFotos as $index => $fotoPath)
                    <div class="foto-item" data-path="{{ $fotoPath }}">
                        <img src="{{ Storage::url($fotoPath) }}" alt="Foto {{ $index + 1 }}" 
                             onclick="showImagePreview('{{ Storage::url($fotoPath) }}')"
                             class="cursor-pointer">
                        <button type="button" 
                                class="delete-btn"
                                onclick="markForDelete(this, '{{ $fotoPath }}')">
