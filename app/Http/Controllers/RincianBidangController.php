...
class RincianBidangController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Validasi untuk update (TANPA transport)
        $request->validate([
            'spd_id' => 'required|exists:spd,id_spd',
            'bendahara_pengeluaran_id' => 'nullable|exists:tb_pegawai,id_pegawai',
            // transport TIDAK divalidasi
        ]);

        $spd = SPD::with('tempatTujuan', 'pelaksanaPerjadin')->findOrFail($request->spd_id);
        
        // Sync ulang RincianBidang dari SPD (TANPA transport)
    ...
    public function getBySpd($spdId)
    {
        ...
        return response()->json([
            ...
            'data' => [
                ...
                // 'uang_transport' => Dihapus - dikwitansi terpisah
                // 'transport' => Dihapus - dikwitansi terpisah
                'total' => $rincian->total,
    ...
    /**
     * CATATAN PENTING:
     * Uang Transport (transport) TIDAK termasuk dalam RincianBidang ini
     * karena akan dikwitansi secara terpisah sesuai kebijakan.
     * 
     * Jika ada kebutuhan untuk mencatat uang transport, buat tabel terpisah
     * atau modul Kwitansi Transport sendiri.
     */
}